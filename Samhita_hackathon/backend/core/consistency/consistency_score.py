...
def get_consistency_score(df, rules=None):
    """
    Calculate consistency score based on configured rules.

    Args:
        df: Input DataFrame
        rules: Optional list of rule dicts. If None, uses default hardcoded rules.

    Returns:
        Tuple of (violation_score: float 0-1, violation_df: DataFrame)
    """
    if not rules:
        return get_consistency_score_default(df)

    # Apply dynamic rules
    try:
        violation_df, violation_summary = apply_rules(df, rules)
        ...
        # Return unique violated rows with metadata
        ...
        raise ValueError(f"Error calculating consistency score: {str(e)}")


def get_consistency_score_default(df):
    """

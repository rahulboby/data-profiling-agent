...
def validate_conditional(df, condition_field, condition_value, then_field, then_value):
    ...
    try:
        ...
        violations['violation_reason'] = (
            f"IF {condition_field}='{condition_value}' THEN {then_field} must be '{then_value}'"
        )
...
def apply_rules(df, rules):
    ...
    Returns:
        Tuple of (all_violations DataFrame, violation_summary dict)
    """
    ...
    for rule in rules:
        ...
        try:
            if rule_type == "Mandatory Field":
                ...
                violations = validate_conditional(
                    df, field, condition_value, then_field, then_value
                )
